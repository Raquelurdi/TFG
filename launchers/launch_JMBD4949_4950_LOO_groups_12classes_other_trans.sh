...
#ngfs=( 0 )
numfeats=$(seq 5 14 | xargs -n 1 -I {} echo "2^"{} | bc)
#res_10kwords res_10kwords_withoutNormalize res_10kwords_normRWs
path_data=work_JMBD4949_4950_loo_groups_trans
for try in $trys; do
    for ngf in "${ngfs[@]}"; do
        for numfeat in $numfeats; do
            ...
            --te_data ${path_data}/tfidf_4949_4950_loo.txt \
            --class_dict ${path_data}/tfidf_4949_4950_loo_classes.txt --LOO true \
            --classes ${classes}
        done
    done
done
